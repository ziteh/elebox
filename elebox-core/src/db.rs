...
pub fn add_db_category(db_path: &str, db_category: &DbCategory) {
    add_db_item::<DbCategory>(db_path, CATEGORIES_BUCKET, db_category);
}
...
pub fn get_category_id(db_path: &str, name: &str) -> Option<String> {
    let db = DB::open(db_path).unwrap();
    let tx = db.tx(false).unwrap();
    let bkt = tx.get_bucket(CATEGORIES_BUCKET).unwrap();
...
pub fn delete_db_categories(db_path: &str, id: &str) -> String {
    delete_db_items(db_path, CATEGORIES_BUCKET, id)
}
...
pub fn get_db_categories(db_path: &str) -> Vec<DbCategory> {
    get_db_items::<DbCategory>(db_path, CATEGORIES_BUCKET)
}
...
pub fn get_db_category_from_id(db_path: &str, id: &str) -> Option<DbCategory> {
    get_db_item::<DbCategory>(db_path, CATEGORIES_BUCKET, id)
}
...
// pub fn get_part_type_from_id(db_path: &str, id: &str) -> Option<Category> {
