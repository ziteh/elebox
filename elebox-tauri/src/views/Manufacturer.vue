...
async function newMfr() {
    await invoke("new_mfr", { name: name.value, alias: alias.value, url: url.value });
...
<template>
    <v-container>
        <v-form fast-fail @submit.prevent>
            <v-container>
                <v-row class="ga-8">
                    <v-text-field label="Name" variant="outlined" v-model="name" placeholder="Texas Instruments"
